...
class AudioSynthesizer:
    ...
    def _synthesize_note(self, note: str, duration: float):
        ...
        if note not in self.note_frequencies:
            print(f"Warning: Unknown note {note}, skipping...")
            self._add_silence(duration)
            return
    ...
    def _synthesize_chord(self, notes: List[str], duration: float):
        ...
        if chord_samples is not None:
            ...
            chord_samples = chord_samples / len(notes)
    ...
    def _generate_tone(self, frequency: float, duration: float) -> np.ndarray:
        ...
        duration_seconds = (duration * 60.0) / self.tempo
    ...
    def _apply_envelope(self, wave: np.ndarray) -> np.ndarray:
        ...
        decay_samples = min(int(0.05 * self.sample_rate), length // 5)   # 50ms decay
    ...
    def save_audio(self, audio_data: np.ndarray, filename: str, format: str = 'mp3'):
        ...
        # Save as WAV first
        wav_filename = filename.replace('.mp3', '.wav').replace('.ogg', '.wav')
        wavfile.write(wav_filename, self.sample_rate, audio_16bit)
        
        # Convert to desired format if not WAV
        if format.lower() in ['mp3', 'ogg']:
            audio_segment = AudioSegment.from_wav(wav_filename)
            final_filename = filename
            audio_segment.export(final_filename, format=format.lower())
            
            # Remove temporary WAV file
            if wav_filename != final_filename:
